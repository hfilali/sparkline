<!DOCTYPE html>
<html lang="en">
<head>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-143071944-1"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());

    gtag('config', 'UA-143071944-1');
    </script>
    <title>Sparkline | L'équipe</title>
    <?php include("includes/header.php"); ?>
</head>        
<body>

<div id="boxed">
<section>	
<?php include("includes/menu.php"); ?>

<div class="col-wrap "><!-- style col-shadow/ col-gray/ testimonial/-->
    <div class="container ">
        <div class="row-fluid">
            <div class="span12 themeapt_animated_text themeapt_text  themeapt_animate_when_almost_visible left-to-right">
                <!--   Big Title wrap -->	
			    <div class="big-title">
                    <h1>Expertises, Passion et Connexion à un réseau de spécialistes pour concrétiser votre projet</h1>
                </div>
                <!--   hr-Start-->			  
			    <div class="hr hr-center"> <!--   hr-center/ hr-left/ hr-right/  hr-short/ hr-light/-->
                    <span class="hr-inner">
				        <span class="hr-inner-style"></span>
				    </span>
			    </div>
			    <!--   hr-End-->	
                <p class="center">
                    Comprendre le parcours et le projet professionnels du Candidat pour cibler le bon projet d'entreprise
                </p>					
            </div><!-- /.span12 -->		
        </div><!-- /.row -->		 
	</div><!-- /.container -->
</div><!-- /.col -->
    
<!-- LES MEMBRES DE L'EQUIPE -->
<div class="col-wrap"><!-- style col-shadow/ col-gray/ bg/ testimonial/-->
	<div class="container ">
        <div class="row-fluid">
            <!-- Hadil Filali -->
            <div class="span4">
                <div class="team services animationBegin">
                    <div class="team-img">
                        <img class='themeapt_animated_image themeapt_image themeapt_animate_when_almost_visible right-to-left' src="images/team/hadil-filali.jpg" alt="Hadil Filali">
                    </div>
                    <div class="team-data">
                        <h2>Hadil Filali</h2>
                        <p>Founder & COO</p>
                        <p>Technical AI Data Recruiter</p>
                    </div>
                    <div class="team-sr">
                        <ul>
                            <li><a target="_blank" href="mailto:hadil.filali@sparkline.fr"  rel="mail"><span class="icon-mail"></a></li>
                            <li><a target="_blank" href="https://fr.linkedin.com/in/hadilfilali"  rel="linkedin"><span class="icon-linkedin"></a></li>
                        </ul>
                        <div class="clear"></div>
                    </div>
                </div><!-- /team-item -->
            </div><!-- /.span4 -->
            
            <!-- David Tonnaire -->
            <div class="span4">
                <div class="team services animationBegin">
                    <div class="team-img">
                        <img class='themeapt_animated_image themeapt_image themeapt_animate_when_almost_visible right-to-left' src="images/team/david tonnaire.jpg" alt="David Tonnaire">
                    </div>
                    <div class="team-data">
                        <h2>David Tonnaire</h2>
                        <p>HR Advisor</p>
                        <p>Recrutement, Culture & Engagement Collaborateur</p>
                    </div>
                    <div class="team-sr">
                        <ul>
                            <li><a target="_blank" href="https://www.linkedin.com/in/david-tonnairerh/"  rel="linkedin"><span class="icon-linkedin"></a></li>
                            <li><a target="_blank" href="https://www.welcomeonboard.link/"  rel="home"><span class="icon-home"></a></li>    
                        </ul>
                        <div class="clear"></div>
                    </div>
                </div><!-- /team-item -->
            </div><!-- /.span4 -->
            
            <!-- Loïc Knuchel-->
            <div class="span4">
                <div class="team services animationBegin">
                    <div class="team-img">
                        <img class='themeapt_animated_image themeapt_image themeapt_animate_when_almost_visible right-to-left' src="images/team/loic-knuchel.jpg" alt="Loïc Knuchel">
                    </div>
                    <div class="team-data">
                        <h2>Loïc Knuchel</h2>
                        <p>Software Engineering Advisor</p>
                        <p>Architecture & Software Engineering at scale</p>
                    </div>
                    <div class="team-sr">
                        <ul>
                            <li><a target="_blank" href="https://fr.linkedin.com/in/loicknuchel"  rel="linkedin"><span class="icon-linkedin"></a></li>
                        </ul>
                        <div class="clear"></div>
                    </div>
                </div><!-- /team-item -->
            </div><!-- /.span4 -->
            
            
            
        </div><!-- /.row -->			 
    </div><!-- /.container -->
</div><!-- /.col -->

<!-- DEUXIEME LIGNE -->
    
<div class="col-wrap"><!-- style col-shadow/ col-gray/ bg/ testimonial/-->
	<div class="container ">
        <div class="row-fluid">
            <!-- Yasir Khan -->
            <div class="span4">
                <div class="team services animationBegin">
                    <div class="team-img">
                        <img class='themeapt_animated_image themeapt_image themeapt_animate_when_almost_visible bottom-to-top' src="images/team/Yasir KHAN, PhD Machine Learning Data Science Big Data.jpg" alt="Yasir Khan, Ph.D">
                    </div>
                    <div class="team-data">
                        <h2>Yasir Khan, PhD</h2>
                        <p>AI Advisor</p>
                        <p>Consulting AI, Data Science & Machine Learning at scale</p>
                    </div>
                    <div class="team-sr">
                        <ul>
                            <li><a target="_blank" href="https://www.linkedin.com/in/yasirkhan/"  rel="linkedin"><span class="icon-linkedin"></a></li>
                        </ul>
                        <div class="clear"></div>
                    </div>
                </div><!-- /team-item -->
            </div><!-- /.span4 --> 
            
            <!-- Prosper Burq-->
            <div class="span4">
                <div class="team services animationBegin">
                    <div class="team-img">
                        <img class='themeapt_animated_image themeapt_image themeapt_animate_when_almost_visible bottom-to-top' src="images/team/prosper-burq.jpg" alt="Prosper Burq">
                    </div>
                    <div class="team-data">
                        <h2>Prosper Burq</h2>
                        <p>Big Data Advisor</p>
                        <p>Acrhitecture Big Data & Machine Learning at scale</p>
                    </div>
                    <div class="team-sr">
                        <ul>
                            <li><a target="_blank" href="https://fr.linkedin.com/in/prosper-burq-47890820"  rel="linkedin"><span class="icon-linkedin"></a></li>
                        </ul>
                        <div class="clear"></div>
                    </div>
                </div><!-- /team-item -->
            </div><!-- /.span4 --> 
            
        </div><!-- /.row -->			 
    </div><!-- /.container -->
</div><!-- /.col -->
    
<?php include("includes/footer.php"); ?>
</section>	
</div>
<?php include("includes/jsfiles.php"); ?>		
<!-- 12/03/17 : DASHBOARD ANALYTICS by Florent Defontis -->
<script type="text/javascript" src="https://cdn.air360tracker.net/air360.min.js"></script>
<script type="text/javascript">
     Air360.init("paa3v08emtjbn59o");
</script>		
</body>
</html>