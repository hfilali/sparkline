<!DOCTYPE html>
<html lang="en">
<head>
<title>Sparkline | 403 Error - Forbidden</title>
<?php include("includes/header.php"); ?>
</head>        
<body>

<div id="boxed">
<section>	
<?php include("includes/menu.php"); ?>
		
<!-- BAR TITRE -->
<div class="title-bar"><!-- style t-shadow/ t-gray/ -->
	<div class="container ">
		<h2 class="page-title">403 Error - Forbidden</h2> <!-- /.page-title -->
    </div> <!-- /.container -->
</div><!-- /.title-bar -->
<!-- FIN BAR TITRE -->

<div class="col-wrap col-shadow col-gray"><!-- style col-shadow/ col-gray/ testimonial/-->
	<div class="container ">
		  <!-- Three columns of text below the carousel -->
		  <div class="row-fluid"><!-- box-style row Class .box-style -->
              <div class="span12 ">
                  <div class="p404 ">
                      <h1>403</h1>
                      <!--   hr-Start-->
                      <div class="hr   hr-center"> <!--   hr-center/ hr-left/ hr-right/  hr-short/ hr-light/-->
                          <span class="hr-inner">
                              <span class="hr-inner-style"></span>
                          </span>
                      </div>
                      <!--   hr-End-->			  
                      <h2>L' accès au fichier requiert une autorisation.</h2>
                      <p class="center p404-box">
                          <a class="button" href="index.php">Retournez à l'accueil</a>
                      </p>
                  </div><!-- /.p404 -->
              </div><!-- /.span4 -->
        </div><!-- /.row -->		  
    </div>
</div><!-- /.col -->	


<?php include("includes/footer.php"); ?>		
	
</section>
	
</div>

<?php include("includes/jsfiles.php"); ?>		
<!-- 12/03/17 : DASHBOARD ANALYTICS by Florent Defontis -->
<script type="text/javascript" src="https://cdn.air360tracker.net/air360.min.js"></script>
<script type="text/javascript">
     Air360.init("paa3v08emtjbn59o");
</script>		
</body>
</html>